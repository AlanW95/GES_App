...
public class UserInterfaceManagerUI : MonoBehaviour
{
    ...
    public void Open_CoachSelectionScreen() { ChangeWindow(HomeScreen); BannerBottom.SetActive(true); /*ChangeWindow(CoachSelectionScreen);*/ Configure_Top_Banner(true); TotalSkillsAddedText.text = accountManager.localUserAccount._skills.Count + " Skills"; HomeScreenExpCountText.text = accountManager.localUserAccount._experiences.Count.ToString(); HomeScreenProjCountText.text = accountManager.localUserAccount._artifacts.Count.ToString(); HomeScreenRefCountText.text = accountManager.localUserAccount._references.Count.ToString(); }
